    ...
    public class PlaylistsEndpoint : BaseEndpoint
    {
        ...
        private void ValidetaNameAndDescription(string name, string description)
        {
            ...
            Contract.Requires<ArgumentNullException>(!string.IsNullOrEmpty(description),
                "description field null or empty");
        ...
        /// <summary>
        ///     You can retrieve a playlist. If the playlist is not public, it must be owned by the user identified by the access
        ///     token.
        /// </summary>
        ...
        /// <summary>
        ///     You can retrieve the subscribed users of any public playlist. If the playlist is private, only the owner can view
        ///     its subscribers.
        /// </summary>
        /// <param name="playlistId">The unique ID for the playlist.</param>
        ...
        ///     than or equal to this value. The maximum permitted value is 200. If a value higher than 200 is specified, no more
        ///     200 results will be returned. Default 20.
        /// </param>
        /// <returns>Returns the list of UserData</returns>
        public async Task<MultipleRootObject<UserData>> GetPlaylistSubscribers(string playlistId, int offset = 0,
            int limit = 20)
        {
        ...
        /// <returns>True if tracks were successfully added to the playlist</returns>
        public async Task<bool> AddTracksToPlaylist(string playlistId, IEnumerable<string> trackIds)
        {
            Contract.Requires<ArgumentNullException>(!string.IsNullOrEmpty(playlistId), "playlistId field is null");
            Contract.Requires<ArgumentNullException>(trackIds != null, "trackIds is null");
            Contract.Requires<ArgumentException>(trackIds.Any(), "No ids in trackIds.");
            Contract.Requires<ArgumentException>(trackIds.Count() <= 500,
                "Maximum of 500 tracks exceeded.");
            var trackIdParameters = new List<KeyValuePair<string, string>>();
